...
const App = ()=> {
  const [dropdown,setDropdown] = useState("cs");
  const [isChecked,setIsChecked] = useState(false);
  const [radio,setdradio] = useState("cs");
  return (
    <div>
   <form>
     <h1>drop down value is: {dropdown}</h1>
      <h1> checkebox is:{isChecked ? "True" : "false"}</h1>
      <h1>Radio button is :{radio}</h1>
    
     <select value={dropdown} onChange={(e)=>{setDropdown(e.target.value)}}>
        <option value="cs">computer sciece</option>
        <option value="ec">eletrical and communication</option>
